# 時系列予測モデルの設定ファイル
# データセット、モデル、ハイパーパラメータの設定を管理

# モデルタイプ別の入力要件定義
need_x_y_mark = ['Autoformer', 'Transformer', 'Informer']  # xとyのマーク（時間特徴量）が必要なモデル
need_x_mark = ['TCN', 'FSNet', 'OneNet', 'iTransformer']   # xのマークのみ必要なモデル
need_x_mark += [name + '_Ensemble' for name in need_x_mark]  # アンサンブル版も追加
...
def get_borders(args):
    """
    オンライン学習用のデータ境界を設定する関数

    Args:
        args: 引数オブジェクト（data, border_type, seq_lenを含む）

    Returns:
        None: args.bordersに境界値を設定
    """
    if args.border_type == 'online':
        if args.data.startswith('ETTh'):
            # ETThデータセット用の境界設定（月単位）
...
def get_hyperparams(data, model, args, reduce_bs=True):
    """
    データセットとモデルに応じたハイパーパラメータを取得する関数

    Args:
        data (str): データセット名
        model (str): モデル名
        args: 引数オブジェクト
        reduce_bs (bool): バッチサイズを削減するかどうか
...
def drop_last_PatchTST(args):
    """
    PatchTSTモデル用のデータ境界調整関数
    バッチサイズに合わせてテストデータの境界を調整する

    Args:
        args: 引数オブジェクト（dataset, borders, seq_len, pred_lenを含む）

    Returns:
        None: args.borders[1][2]を調整
    """
    # データセットに応じたバッチサイズ設定
