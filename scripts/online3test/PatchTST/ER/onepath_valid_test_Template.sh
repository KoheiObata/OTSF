...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/PatchTST/ER/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$online_learning_rate" "$optim" "$buffer_size" "$mini_batch" "$er_alpha" "$iterations"
	...
	best_params_str=$(python scripts/hyperparameter_search.py \
		...
		--search_params "online_learning_rate=$online_learning_rate" "optim=$optim" "buffer_size=$buffer_size" "mini_batch=$mini_batch" "er_alpha=$er_alpha" \
		--return_param online_learning_rate er_alpha \
		)
	...
	read -r best_olr best_er_alpha <<< "$best_params_str"
	...
	echo "--------------------------------"
	sh scripts/online3test/PatchTST/ER/onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$best_er_alpha" "$final_iterations"
done
done
done
done
