...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/PatchTST/EWC/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$online_learning_rate" "$optim" "$buffer_size" "$mini_batch" "$ewc_lambda" "$iterations"
