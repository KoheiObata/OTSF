...
do
	...
	buffer_size="64"
	mini_batch="4"
	ewc_lambda="10000 100000 1000000 10000000"
	...
	echo "--------------------------------"
	sh scripts/online3test/PatchTST/EWC/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$ewc_lambda" "$iterations"
	...
	best_params_str=$(python scripts/hyperparameter_search.py \
		...
		--search_params "online_learning_rate=$best_olr" "optim=$optim" "buffer_size=$buffer_size" "mini_batch=$mini_batch" "ewc_lambda=$ewc_lambda" \
		--return_param ewc_lambda \
		)
	...
	# read command to assign space-separated string to each variable
	read -r best_ewc_lambda <<< "$best_params_str"
	...
	echo "--------------------------------"
	sh scripts/online3test/PatchTST/EWC/onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$best_ewc_lambda" "$final_iterations"
done
done
done
done
