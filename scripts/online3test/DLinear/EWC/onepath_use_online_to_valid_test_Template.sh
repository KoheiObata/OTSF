...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/DLinear/EWC/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$ewc_lambda" "$iterations"
	...
	echo "--------------------------------"
	sh scripts/online3test/DLinear/EWC/onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$best_ewc_lambda" "$final_iterations"
done
done
done
done
