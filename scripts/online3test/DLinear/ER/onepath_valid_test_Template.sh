...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/DLinear/ER/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$online_learning_rate" "$optim" "$buffer_size" "$mini_batch" "$er_alpha" "$iterations"
	...
	echo "--------------------------------"
	sh scripts/online3test/DLinear/ER/onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$best_er_alpha" "$final_iterations"
done
done
done
done
