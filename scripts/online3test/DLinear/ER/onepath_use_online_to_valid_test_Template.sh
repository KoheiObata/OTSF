...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/DLinear/ER/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$er_alpha" "$iterations"
