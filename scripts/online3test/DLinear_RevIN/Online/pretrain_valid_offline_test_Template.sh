...
do
	...
	echo "iterations: $iterations"
	echo "--------------------------------"
	sh scripts/online3test/DLinear_RevIN/Online/validation_pretrain_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$learning_rate" "$optim" "$batch_size" "$online_learning_rate" "$iterations"
	...
	echo "iterations: $final_iterations"
	echo "--------------------------------"
	sh scripts/online3test/DLinear_RevIN/Online/pretrain_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_lr" "$optim" "$batch_size" "$best_olr" "$final_iterations"
done
done
done
done