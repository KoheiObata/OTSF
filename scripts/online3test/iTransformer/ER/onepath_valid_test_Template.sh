...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/iTransformer/ER/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$online_learning_rate" "$optim" "$buffer_size" "$mini_batch" "$er_alpha" "$iterations"
