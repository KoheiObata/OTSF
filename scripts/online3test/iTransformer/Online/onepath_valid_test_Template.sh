...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/iTransformer/Online/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$online_learning_rate" "$optim" "$iterations"
	...
	echo "--------------------------------"
	sh scripts/online3test/iTransformer/Online/onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$final_iterations"
done
done
done
done
