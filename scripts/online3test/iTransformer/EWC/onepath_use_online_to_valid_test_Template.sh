...
do
	...
	echo "--------------------------------"
	sh scripts/online3test/iTransformer/EWC/validation_onepath_Template.sh "$data" "$seq_len" "$pred_len" "$devices" "$best_olr" "$optim" "$buffer_size" "$mini_batch" "$ewc_lambda" "$iterations"
