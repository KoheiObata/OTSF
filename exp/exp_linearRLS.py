...
from models.HybridPredictor import RLSLinearPredictorShare


warnings.filterwarnings('ignore')

class Exp_LinearRLS(Exp_Online):
    ...
    def __init__(self, args):
        ...
        # RLS predictor
        self.model = RLSLinearPredictorShare(
            seq_len=self.args.seq_len,
            pred_len=self.args.pred_len,
            num_variates=self.args.enc_in,
            lambda_forget=self.args.forget_factor,
            device=self.device,
            update_batch=self.args.update_batch,
            revin=self.args.revin
        )
        self.initial_train_batch = self.args.initial_train_batch

        # Configure data acquisition for sequential learning
        self.wrap_data_kwargs.update(gap=self.args.pred_len)


    def _update(self, batch, current_batch=None):
        """
        Online update
        """

        if batch[0].dim() == 2:
            x = batch[0].unsqueeze(0)  # [1, L, D]
            y = batch[1].unsqueeze(0)  # [1, H, D]
        else:
            x = batch[0]  # [1, L, D]
            y = batch[1]  # [1, H, D]

        x = x.transpose(1, 2)  # [B, L, D]
        y = y.transpose(1, 2)  # [B, H, D]

        self.model.update(X_t=x, Y_t_true=y)


    def _get_initial_data(self, online_loader):
        # Randomly get initial_train_batch data from online_loader
        initial_data_x = []
        initial_data_y = []

        # Get dataset length
        dataset_length = len(online_loader.dataset)

        # Generate random indices
        random_indices = torch.randperm(dataset_length)[:self.initial_train_batch]

        # Get randomly selected data
        for idx in random_indices:
            batch_data, _ = online_loader.dataset[idx]
            batch_x, batch_y, batch_x_mark, batch_y_mark, index = batch_data

            # Add batch dimension
            if batch_x.dim() == 2:
                batch_x = batch_x.unsqueeze(0)  # [L, D] -> [1, L, D]
                batch_y = batch_y.unsqueeze(0)  # [H, D] -> [1, H, D]

            initial_data_x.append(batch_x)
            initial_data_y.append(batch_y)

        # Concatenate data
        X_init = torch.cat(initial_data_x, dim=0)  # [B, L, D]
        Y_init = torch.cat(initial_data_y, dim=0)  # [B, H, D]

        # Convert to format expected by RLSLinearPredictorShare: [B, L, D] -> [B, D, L]
        X_init = X_init.transpose(1, 2)  # [B, D, L]
        Y_init = Y_init.transpose(1, 2)  # [B, D, H]
    ...
    def online(self, phase='test', savename='', show_progress=False):
        ...
        if self.phase == 'train' and self.initial_train_batch > 0:
            # Randomly get initial_train_batch data from online_loader
            X_init, Y_init = self._get_initial_data(online_loader)
            self.model.initialize_with_data(X_init, Y_init)
            print(f"Initialized RLS predictor with {len(X_init)} random samples")
        ...
        for i, (recent_batch, current_batch) in enumerate(online_loader):
            ...
            # Online update with recent_batch
            if (i+1) % self.args.update_interval == 0:
                self._update(recent_batch, current_batch=None)
