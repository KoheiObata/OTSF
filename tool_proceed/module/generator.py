...
from tool_proceed.module.base import Adaptation


def clip(x, max_norm=1):
    """
    入力テンソルxのノルムをmax_norm以下にクリップする。
    - 大きすぎる値による不安定化を防ぐ。
    """
...
class Bottleneck(nn.Module):
    """
    適応パラメータ生成用のボトルネックMLP。
    - shared: 全層で重み共有するか
    - need_bias: バイアス項を出力するか
    - forward: 入力から適応パラメータを生成
    """
...
class AdaptGenerator(nn.Module):
    """
    モデル全体の各適応層に対応するBottleneckを管理するクラス。
    - 各層ごとにBottleneckを持ち、forwardで全層分の適応パラメータをまとめて生成。
    """
    def __init__(self, backbone: nn.Module, concept_features: int, activation=nn.LeakyReLU,
        ...
        # backbone内の各Adaptation層を探索し、出力次元ごとにグループ化
