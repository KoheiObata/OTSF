...
from tool_proceed.module import ssf, up
from tool_proceed.module.up import LayerNorm, BatchNorm1d, RevIN, _RevIN


def add_down_up_(parent_module: nn.Module, module_name: str, freeze_weight: bool,
             merge_weights=False, load_weights=True, **kwargs):
    """
    指定した親モジュールのサブモジュール(module_name)をDown_Up適応層に置き換える。
    - Linear/Conv1D/BatchNorm/LayerNorm/RevINなど多様な層に対応。
    - 既存の重み・バイアスを新モジュールにコピー。
    """
...
class Down_Up(up.Adaptation_Up):
    """
    入力側・出力側両方にスケーリング（scale2, scale）を適用できる適応層。
    - assign_adaptationで適応パラメータをセット。
    - _merge, _ssf_input, _ssfで重み・バイアス・入力の適応処理。
    """
