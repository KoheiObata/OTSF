...
import tool_btoa.lib.dist as dist
import tool_btoa.lib.utils as utils
from tool_btoa.lib.flows import FactorialNormalizingFlow
from tool_btoa.elbo_decomposition import elbo_decomposition
...
class VAE(nn.Module):
    def __init__(self, z_dim, args=None, use_cuda=False, prior_dist=dist.Normal(), q_dist=dist.Normal(),
        ...
        elif self.args.data == 'ieee_small' or self.args.data == 'ieee_big' or self.args.data == 'dalia':
            self.encoder = ConvEncoder_ieeesmall(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_ieeesmall(z_dim)
        elif self.args.data == 'ecg':
    ...
    def encode(self, x):
        ...
        # print('z_params.shape',z_params.shape, 'x.size(0)',x.size(0), 'self.z_dim',self.z_dim, 'self.q_dist.nparams',self.q_dist.nparams)
...
def train_VAE(train_loader, args, DEVICE):
    ...
    while iteration < num_iterations:
        for i, x in enumerate(train_loader):
            ...
            # print('x.shape',x.shape, 'dataset_size', dataset_size)
