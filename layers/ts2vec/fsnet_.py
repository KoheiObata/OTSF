...
class SamePadConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, groups=1, gamma=0.9):
        ...
        # Explicitly specify device (initialize on CPU here to move later with .to(device))
    ...
    def fw_chunks(self):
        # First, move self.grads and self.f_grads to correct device
        # Use device of other components as reference
        device = next(self.controller.parameters()).device
        self.grads = self.grads.to(device)
        self.f_grads = self.f_grads.to(device)
        ...
        self.calib_w = self.calib_w.to(device)
        self.calib_b = self.calib_b.to(device)
        self.calib_f = self.calib_f.to(device)
        ...
        if not hasattr(self, 'q_ema'):
            setattr(self, 'q_ema', torch.zeros(*q.size()).float().to(device))
        else:
        ...
        if self.trigger == 1:
            ...
            old_w = ww @ idx
            # write memory
            s_att = torch.zeros(att.size(0)).to(device)
            s_att[idx.squeeze().long()] = v.squeeze()
            W = old_w @ s_att.unsqueeze(0)
            mask = torch.ones(W.size()).to(device)
